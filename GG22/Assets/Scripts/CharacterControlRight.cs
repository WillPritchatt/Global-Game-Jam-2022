...
public class CharacterControlRight : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        controls.Right.Enable();
        controls.Right.Move.performed += ctx => move = ctx.ReadValue<Vector2>();
        controls.Right.Move.canceled += ctx => move = Vector2.zero;
        controls.Right.ClawSwap.performed += ctx => clawChange = ctx.ReadValue<Vector2>().y;
        controls.Right.Jump.performed += ctx => jump = ctx.ReadValue<float>();
        controls.Right.Strike.performed += ctx => Strike();
    }
